...
class RangedAttacker extends CCB {
    constructor(heal, target) {
        super();
        this.role = 'RangedAttacker';
        if (heal) {
            this.body = target == "middle" ?
                [constants.MOVE, constants.RANGED_ATTACK, constants.MOVE, constants.HEAL] :
                // [constants.MOVE, constants.MOVE, constants.MOVE, constants.MOVE, constants.MOVE, constants.MOVE, constants.MOVE, constants.MOVE, constants.MOVE, constants.RANGED_ATTACK, constants.MOVE, constants.HEAL];
                [constants.MOVE, constants.MOVE, constants.MOVE, constants.RANGED_ATTACK, constants.MOVE, constants.RANGED_ATTACK, constants.MOVE, constants.RANGED_ATTACK, constants.MOVE, constants.HEAL];
        } else {
            this.body = target == "middle" ?
                [constants.MOVE, constants.RANGED_ATTACK] :
                [constants.MOVE, constants.MOVE, constants.MOVE, constants.MOVE, constants.MOVE, constants.MOVE, constants.MOVE, constants.MOVE, constants.MOVE, constants.RANGED_ATTACK, constants.MOVE, constants.RANGED_ATTACK];
        }
        this.status = 'waiting';
        this.target = target;
    }
        ...
        if (this.creep) {
            if (this.status == 'waiting') {
                ...
                // } else if (fighting) {
                    ...
                    this.creep.moveTo(this.target == "north" ? north_point : this.target == "south" ? south_point : middle_point);
            ...
            if (stat.enemies.length > 0) {
                ...
                if (closest) {
                    ...
                    if (closest_dist < 3 || this.creep.body[0].hits < 20 && this.creep.body.find(part => part.type == constants.HEAL)) {
                        this.creep.moveTo(my_spawn);
                        this.creep.rangedAttack(closest);
                    } else if (closest_dist > 30 || closest_dist > utils.getRange(this.creep, enemy_spawn) + 5) {
...
export function loop() {
    ...
    var stat = {};
    stat.enemies = utils.getObjectsByPrototype(prototypes.Creep).filter(creep => !creep.my && creep.body.find(part => part.type == constants.ATTACK || part.type == constants.RANGED_ATTACK || part.type == constants.HEAL) && creep.exists);
    ...
    if (stat.enemies.find(enemy => Math.abs(enemy.x - my_spawn.x) < 10)) {
        ccb_list.push(new RangedAttacker(false, "middle"));
        spawn(ccb_list[ccb_list.length - 1]);
    } else {
        var spawning = false;
        for (var ccb of ccb_list) {
        ...
        if (!spawning) {
            // ccb_list.push(new RangedAttacker(false, "north"));
            // ccb_list.push(new RangedAttacker(false, "south"));
            ccb_list.push(new RangedAttacker(true, "north"));
            ccb_list.push(new RangedAttacker(true, "south"));
